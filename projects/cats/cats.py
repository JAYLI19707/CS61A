...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def paragraph_select(k):
        a=split(k)
        orginal=[lower(remove_punctuation(x)) for x in a]
        for x in orginal :
            for y in subject:
                if x==y:
                    return True
...
def accuracy(typed, source):
    ...
    if len(typed_words)>len(source_words):
        match=count /len(typed_words)*100
        ...
        if len(typed_words)<len(source_words):
            ...
            match=count/len(source_words)*100
    return match
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    last=0
    number=[typed[i:i+5] for i in range(0,len(typed),5)]
    if len(number)==0:
        return 0.0
    if len(number[0])<5 :
        count=len(number[0])/5
    elif  len(number[-1])<5:
        last=len(number[-1])/5
        count=len(number)-1
        match=(count+last)*(60/elapsed)
        return match
    else:
        count=len(number)
    match=count*(60/elapsed)
    return match
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    k=[(x,diff_function(typed_word,x,limit)) for x in word_list if diff_function(typed_word, x, limit) <= limit]
    if  k==[]:
        return typed_word
    for x in word_list:
            if typed_word == x:
                return  x
    return min(k,key=lambda pair:pair[1])[0]
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit < 0:  # 如果已经超过限制，返回大于 limit 的值
        return limit + 1
    if not typed:
        return len(source)
    if not source:
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    count = 0
    k=min(len(typed),len(source))
    for i in range(k):
        if typed[i]==source[i]:
            count+=1
        else:
            break
    match=count/len(source)
    upload({'id': user_id, 'progress': match})
    return match
    # END PROBLEM 8
...
def fastest_words(match):
    ...
    total=[[]for i in player_indices] 

    for i in word_indices:
        idx=min(player_indices,key = lambda x:time(match,x,i))
        total[idx].append(get_word(match,i))
    return total
        # END PROBLEM 10
